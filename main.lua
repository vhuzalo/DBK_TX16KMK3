...
local popup_button_positions = nil  -- 存储弹窗按钮位置信息

-- 多模型日志查看相关变量
local log_view_mode = 0           -- 子模式标志：0=模型列表，1=飞行记录列表，2=飞行记录详情
local model_list = {}             -- 模型列表缓存
local selected_model_index = 1    -- 选中的模型索引
local viewing_model_name = ""     -- 正在查看的模型名称
local viewing_log_data = {}       -- 正在查看的模型的日志数据
local viewing_fly_number = 0      -- 正在查看的模型的飞行次数
local model_index_file = "/WIDGETS/Mini/logs/System/model_index.txt"
...
local current_flight_max_current = 0  -- 记录当前飞行的最大电流值
local led_cache = { last_start_color = 0, last_end_color = 0 }  -- LED缓存，用于优化LED更新

-- RPM数据收集相关变量
local rpm_buffer = {}           -- RPM数据缓冲区
local rpm_collect_timer = 0     -- RPM收集计时器（秒）
local rpm_collect_interval = 2  -- RPM收集间隔（秒）- 2秒记录一次，曲线更精确
local rpm_file_path = ""        -- RPM文件路径
local rpm_start_time = ""       -- RPM收集开始时间

-- 电流数据收集相关变量
local current_buffer = {}       -- 电流数据缓冲区
local current_start_time = ""   -- 电流收集开始时间

-- 电压数据收集相关变量
local voltage_buffer = {}       -- 电压数据缓冲区
local voltage_start_time = ""   -- 电压收集开始时间
    ...
    if file_info and file_info.size > 0 then
        ...
        if file_obj then
            ...
            while start_pos <= #content do
                ...
                local model_name = string.gsub(line, "\r", "")  -- 移除可能的\r
...
local function create(zone, options)
    ...
    default_pic_obj = Bitmap.open("/WIDGETS/DBK_T16SMK3Mini/image/default.png")
    hold1_pic_obj = Bitmap.open("/WIDGETS/DBK_T16SMK3Mini/image/hold1.png")
    hold2_pic_obj = Bitmap.open("/WIDGETS/DBK_T16SMK3Mini/image/hold2.png")

    -- 自动将当前模型添加到索引（用于多模型日志查看）
...
local function get_total_flight_count(model_name)
    ...
    local total_file_path = "/WIDGETS/DBK_T16SMK3Mini/logs/System/totalall_[" .. safe_model_name .. "].txt"
...
local function scan_model_logs()
    local models = load_model_index()
    local model_list = {}
    local model_flight_counts = {}  -- 存储每个模型的总飞行次数

    -- 初始化所有模型的飞行次数为0（过滤掉Rotorflight）
    for i = 1, #models do
    ...
    for day_offset = 0, 29 do
        ...
        for i = 1, #models do
            ...
            if model_name ~= ">Rotorflight" and model_name ~= "" then
                ...
                local file_path = "/WIDGETS/DBK_T16SMK3Mini/logs/" .. file_name
    ...
    -- 将model_index中的所有模型添加到列表（即使飞行次数为0，但排除Rotorflight）
...
local function write_rpm_data(model_name, flight_num, start_time, end_time, rpm_data)
    ...
    if file_obj then
        ...
        for i = 1, #rpm_data do
            ...
            rpm_line = rpm_line .. tostring(rpm_data[i])
        end
        io.write(file_obj, rpm_line .. "\n")
...
local function write_current_data(model_name, flight_num, start_time, end_time, current_data)
    ...
    if file_obj then
        ...
        for i = 1, #current_data do
            ...
            current_line = current_line .. string.format("%.1f", current_data[i])
        end
        io.write(file_obj, current_line .. "\n")
...
local function write_voltage_data(model_name, flight_num, start_time, end_time, voltage_data)
    ...
    if file_obj then
        ...
        -- 写入电压数据（逗号分隔，保留2位小数）
        local voltage_line = ""
        for i = 1, #voltage_data do
            if i > 1 then
                voltage_line = voltage_line .. ","
            end
            voltage_line = voltage_line .. string.format("%.2f", voltage_data[i])
        end
        io.write(file_obj, voltage_line .. "\n")
...
local function draw_ring_progress(xs, ys, value, max_value, size)
    -- ========== 圆环基本参数 ==========
    local radius = size                      -- 圆环外半径
    local ring_width = 8                     -- 圆环宽度（固定8像素）
    local segments = 20                      -- 完整360度的总段数（用于计算每段角度）
    local gap = 3                            -- 每段之间的间隙角度（度）
    local angle_coverage = 270               -- 圆环覆盖的总角度（270度）
    local segments_to_draw = 15              -- 实际绘制的段数 (20 * 270/360 = 15)
    local start_angle = -135                 -- 起始角度（-135度=左下位置）
    local gray_color = lcd.RGB(80, 80, 80)   -- 未激活段的灰色

    -- ========== 第一步：限制value在有效范围内 ==========
    value = math.max(0, math.min(max_value, value))

    -- ========== 第二步：计算进度百分比和激活段数 ==========
    local progress_percent = value / max_value              -- 进度百分比（0.0-1.0）
    local active_segments = math.floor(segments_to_draw * progress_percent)  -- 需要激活的段数

    -- ========== 第三步：逐段绘制圆环 ==========
    for i = 0, segments_to_draw - 1 do
        -- 计算当前段的起始和结束角度
        -- 每段角度 = 360° / 20段 = 18°
        -- 加上起始偏移角度（-135°），再加减间隙的一半（±1.5°）
        local angle_start = (360 / segments) * i + start_angle + gap / 2
        local angle_end = (360 / segments) * (i + 1) + start_angle - gap / 2

        local segment_color = gray_color  -- 默认为灰色（未激活）

        -- ========== 第四步：判断当前段是否激活，并计算渐变色 ==========
        if i < active_segments then
            -- 计算当前段在整个进度条中的位置百分比（0-100）
            -- 例如：第0段 → 0%，第7段 → 50%，第14段 → 100%
            local seg_percent = (i / (segments_to_draw - 1)) * 100

            -- 计算渐变色：从绿色(0%)到红色(100%)
            -- RGB色彩空间：绿色(0,255,0) → 黄色(255,255,0) → 红色(255,0,0)
            -- 红色分量：从0递增到255（seg_percent * 2.55）
            -- 绿色分量：从255递减到0（255 - seg_percent * 2.55）
            -- 蓝色分量：固定为0
            local r = math.floor(seg_percent * 2.55)         -- 红色：0→255
            local g = math.floor(255 - (seg_percent * 2.55)) -- 绿色：255→0
            local b = 0                                       -- 蓝色：固定0
            segment_color = lcd.RGB(r, g, b)
        end

        -- ========== 第五步：绘制实心圆环段 ==========
        -- drawAnnulus参数：中心X, 中心Y, 内半径, 外半径, 起始角度, 结束角度, 颜色
        -- 内半径 = 外半径 - 圆环宽度
...
local function rqly_signal_bars_ladder(xs, ys, rqly_percent, default_color, size)
    -- ========== 参数说明 ==========
    -- xs, ys: 起始位置（左下角）
    -- rqly_percent: 信号强度百分比 (0-100)
    -- default_color: 未激活柱条的颜色
    -- size: 整体大小缩放因子（建议值：1.0）

    -- ========== 基本参数 ==========
    local bar_count = 6                          -- 柱条数量
    local bar_width = math.floor(6 * size)       -- 每个柱条的宽度
    local bar_spacing = math.floor(2 * size)     -- 柱条之间的间隙
    local base_height = math.floor(4 * size)     -- 最矮柱条的高度
    local height_increment = math.floor(4 * size) -- 每个柱条递增的高度

    -- ========== 限制信号百分比范围 ==========
    rqly_percent = math.max(0, math.min(100, rqly_percent))

    -- ========== 计算激活的柱条数量 ==========
    -- 每个柱条代表约16.67%的信号强度（100/6）
    ...
    for i = 1, bar_count do
        ...
        local bar_y = ys - bar_height  -- 从底部向上绘制
        ...
        if rqly_percent > 0 and i <= active_bars then
            if i == 1 then
                bar_color = RED                      -- 第1格：红色（信号很弱）
            elseif i == 2 then
                bar_color = ORANGE                   -- 第2格：橙色
            elseif i == 3 then
                bar_color = YELLOW                   -- 第3格：黄色
            elseif i == 4 then
                bar_color = lcd.RGB(173, 255, 47)    -- 第4格：黄绿色
            elseif i == 5 then
                bar_color = lcd.RGB(0, 255, 0)       -- 第5格：绿色
            else
                bar_color = GREEN                    -- 第6格：深绿色（信号满格）
            end
        end
...
local function draw_gauge_meter(xs, ys, value, max_value, size, color, bg_color)
    local radius = size
    local start_angle = 180  -- 左侧
    local end_angle = 270    -- 上方 (1/4圈)
    ...
    local scale_start = 190  -- 刻度起始角度（更紧凑）
    local scale_end = 260    -- 刻度结束角度（更紧凑）
    ...
    -- 绘制1/4圆边缘轮廓线（分段渐变色，从红到绿）
    local segments = 45  -- 分成45段（360-270=90度）
    local arc_start_angle = 270  -- 上方
    local arc_end_angle = 360    -- 右侧（0度）
    ...
    --     lcd.drawArc(xs, ys, radius * 0.87, arc_start, arc_end, color)
    --     lcd.drawArc(xs, ys, radius * 0.85, arc_start, arc_end, color)
    --     lcd.drawArc(xs, ys, radius * 0.83, arc_start, arc_end, color)
    -- end
    ...
    -- 沿着60刻度方向移动圆心
    local offset_angle = 232  -- 60刻度的角度方向
    local offset_rad = math.rad(offset_angle)
    local offset_distance = radius * 0.08 -- 偏移距离 
    ...
    -- 指针参数 
    local pointer_length = radius * 0.60 --  长度
...
local function draw_digital_display(x, y, value, num_digits, decimal_places, digit_size, color)
    -- 计算七段数码管的各部分尺寸
    local seg_width = digit_size * 0.6        -- 横段宽度
    local seg_height = digit_size * 0.5       -- 竖段高度
    local seg_thickness = digit_size * 0.15   -- 段的粗细
    local digit_spacing = digit_size * 1.2    -- 数字之间的间距
    local gray_color = lcd.RGB(80, 80, 80)    -- 前导0的灰色
    local dot_size = seg_thickness            -- 小数点大小

    -- ========== 第一步：分离整数和小数部分 ==========
    local multiplier = math.pow(10, decimal_places)  -- 10的decimal_places次方
    local int_part = math.floor(value)               -- 整数部分
    -- 小数部分转换为整数进行处理（如0.23转为23），并四舍五入
    local dec_part = math.floor((value - int_part) * multiplier + 0.5)

    -- ========== 第二步：处理四舍五入导致的进位 ==========
    -- 例如：9.999四舍五入后，小数部分变成1000（>=100），需要进位
    if dec_part >= multiplier then
        int_part = int_part + 1  -- 整数部分+1
        dec_part = 0             -- 小数部分归零
    end

    -- ========== 第三步：限制整数部分的最大值 ==========
    -- 例如：num_digits=3时，最大值为999
    local max_int_value = math.pow(10, num_digits) - 1
    int_part = math.min(max_int_value, int_part)

    -- ========== 第四步：计算整数部分的实际位数 ==========
    -- 用于判断哪些是前导0（灰色），哪些是有效数字（彩色）
    local int_value_digits = 0
    if int_part == 0 then
        int_value_digits = 1  -- 特殊情况：0也算1位
    else
        -- 使用对数计算位数：log10(123) = 2.09... → floor(2.09) + 1 = 3位
        int_value_digits = math.floor(math.log(int_part) / math.log(10)) + 1
    end

    local current_x = x  -- 当前绘制位置的X坐标
    ...
    for i = 0, num_digits - 1 do
        ...
        -- 判断当前位是否为前导0
        local digit_color = gray_color       -- 默认为灰色
        local is_leading_zero = true         -- 默认是前导0
        if i >= (num_digits - int_value_digits) then
            -- 如果当前位索引 >= 前导0的数量，则为有效数字
            digit_color = color              -- 使用指定颜色
            is_leading_zero = false          -- 标记为有效数字
        end

        -- 绘制当前数字（所有数字都使用统一间距，保证对齐）
    ...
    for i = 0, decimal_places - 1 do
        ...
        current_x = current_x + digit_spacing  -- 使用正常间距
    end
end
...
local function draw_time_display(x, y, hours, minutes, digit_size, color)
    ...
    local seg_thickness = digit_size * 0.15   -- 段的粗细
    local digit_spacing = digit_size * 1.2    -- 数字之间的间距
    local gray_color = lcd.RGB(80, 80, 80)    -- 灰色
    local colon_size = seg_thickness * 1.5    -- 冒号大小
...
local function draw_power_gauge(center_x, center_y, radius, power_value, max_power, gauge_color, needle_color)
    -- ========== 第一步：参数初始化和范围限制 ==========
    -- 限制最大功率显示范围（单位：瓦，虽然实际用于温度显示）
    local display_max = max_power or 5000  -- 默认最大5000W (5kW)
    power_value = math.max(0, math.min(display_max, power_value))

    -- ========== 第二步：定义弧形角度参数 ==========
    -- 270度弧：从225度（左下）顺时针到-45度/135度（右下）
    -- 角度说明：0度=右侧，90度=下方，180度=左侧，270度=上方
    local start_angle = 225  -- 起始角度（左下位置）
    local end_angle = 135    -- 结束角度（右下位置）
    local total_sweep = 270  -- 总扫描角度（270度）

    -- ========== 第三步：绘制270度弧形边框 ==========
    -- 由于跨越360度边界，需要分两段绘制
    -- 第一段：从225度到360度（左下到右侧）
    ...
    -- ========== 第四步：绘制刻度线和刻度值 ==========
    -- 11个刻度（0%, 10%, 20%...100%），每个刻度间隔27度（270度/10=27度）
    local num_ticks = 11  -- 0%, 10%, 20% ... 100%
    for i = 0, num_ticks - 1 do
        -- 从225度开始，顺时针方向每次减少27度
        -- i=0: 225度（左下，0%）
        -- i=5: 225-135=90度（下方，50%）
        -- i=10: 225-270=-45度/315度（右下，100%）
        local angle_deg = start_angle - (i * 27)
        local angle_rad = math.rad(angle_deg)  -- 转换为弧度

        -- 计算刻度线的起点和终点（径向线）
        local tick_start_r = radius - 5   -- 刻度线外端（距离圆心较远）
        local tick_end_r = radius - 12    -- 刻度线内端（距离圆心较近）
        -- 使用极坐标转换为笛卡尔坐标
        -- 注意：使用 -sin 是因为屏幕Y轴向下为正
        ...
        if i % 2 == 0 then
            -- 计算刻度对应的数值
            local value = (display_max / 10) * i  -- i=0→0, i=2→20%, i=4→40%...
            local label = ""

            -- 智能格式化：>=1000显示为"Xk"，<1000显示完整数值
            if value >= 1000 then
                -- 例如：5000 → "5k"，2500 → "2k"（向下取整）
    ...
    -- ========== 第五步：计算并绘制指针 ==========
    -- 将当前数值映射到角度（0-100%映射到225度顺时针旋转到-45度）
    local percentage = power_value / display_max  -- 当前值百分比（0-1）
    local needle_angle_deg = start_angle - (percentage * total_sweep)  -- 指针角度
    local needle_angle_rad = math.rad(needle_angle_deg)  -- 转换为弧度

    -- 计算指针末端位置
    local needle_length = radius - 15  -- 指针长度（略短于半径）
...
local function draw_rpm_chart(model_name, flight_num, square_color, value_color)
    ...
    -- 读取RPM数据
    local rpm_data = load_rpm_data(model_name, flight_num)
    -- 读取电流数据
    local current_data = load_current_data(model_name, flight_num)
    -- 读取电压数据
    local voltage_data = load_voltage_data(model_name, flight_num)
    ...
    if #rpm_values > 1 then
        ...
        for i = 1, #rpm_values - 1 do
            local x1 = chart_x + (i - 1) * x_step
            local y1 = chart_y + chart_height - ((rpm_values[i] - y_axis_min_rpm) / y_axis_range_rpm * chart_height)
            local x2 = chart_x + i * x_step
            local y2 = chart_y + chart_height - ((rpm_values[i + 1] - y_axis_min_rpm) / y_axis_range_rpm * chart_height)
    ...
    if #current_values > 1 then
        ...
        for i = 1, #current_values - 1 do
            local x1 = chart_x + (i - 1) * x_step
            local y1 = chart_y + chart_height - ((current_values[i] - y_axis_min_current) / y_axis_range_current * chart_height)
            local x2 = chart_x + i * x_step
            local y2 = chart_y + chart_height - ((current_values[i + 1] - y_axis_min_current) / y_axis_range_current * chart_height)
    ...
    if #voltage_values > 1 then
        ...
        for i = 1, #voltage_values - 1 do
            local x1 = chart_x + (i - 1) * x_step
            local y1 = chart_y + chart_height - ((voltage_values[i] - y_axis_min_voltage) / y_axis_range_voltage * chart_height)
            local x2 = chart_x + i * x_step
            local y2 = chart_y + chart_height - ((voltage_values[i + 1] - y_axis_min_voltage) / y_axis_range_voltage * chart_height)
    ...
    -- 计算飞行总时长（秒）
    local total_seconds = (data_count - 1) * 2  -- 2秒间隔
    ...
    for i = 0, data_count - 1 do
        ...
        -- 每个数据点都绘制刻度线
        lcd.drawLine(x, chart_y + chart_height, x, chart_y + chart_height + 3, SOLID, square_color)

        -- 每30秒（15个数据点）显示一次时间标签
...
local function refresh(widget, event, touchState)
            ...
            elseif touchState then
                ...
                if popup_button_positions then
                    ...
                    elseif touchState.x >= pos.col3_x and touchState.x <= (pos.col3_x + pos.button_width) and
                        ...
                        -- 直接显示最新一次飞行的详细记录
                        if fly_number > 0 and log_data[fly_number] then
                            display_mode = 1
                            log_view_mode = 2  -- 直接进入详情模式
                            viewing_model_name = cached_model_name
                            viewing_log_data = log_data
                            viewing_fly_number = fly_number
                            sele_number = fly_number  -- 选择最新的飞行记录
                        else
                            -- 没有日志数据，进入日志列表模式
                            display_mode = 1
                            log_view_mode = 1
    ...
    if not bg_pic_obj then
        bg_pic_obj = Bitmap.open("/WIDGETS/DBK_T16SMK3Mini/image/background.png")
    end
    if bg_pic_obj then
    ...
    -- 根据电压设置颜色
    local tx_color = value_color  -- 默认颜色
    ...
    -- 检测遥测信号初始化和模型名称变化，加载飞行记录
    -- 注意：在查看日志模式下（display_mode == 1）不重新加载，避免清空正在查看的数据
    ...
    if current_model_name ~= cached_model_name and current_model_name ~= "" and display_mode ~= 1 then
        ...
        cached_pic_path = "/WIDGETS/DBK_T16SMK3Mini/modelImage/"..string.sub(cached_model_name, 2)..".png"
        should_load_log = true
    end
    ...
    if should_load_log and current_model_name and current_model_name ~= "" then
        ...
        local new_file_name = "[".. safe_model_name .."]" ..
            ...
            string.format("%02d", getDateTime().mon) ..
            string.format("%02d", getDateTime().day) .. ".log"
        local new_file_path = "/WIDGETS/DBK_T16SMK3Mini/logs/" .. new_file_name
    ...
    if write_en_flag and fly_number < 57 and second[1] > 30 then
        ...
        -- 写入RPM数据（在日志写入成功后）
        if #rpm_buffer > 0 then
            write_rpm_data(cached_model_name, fly_number, rpm_start_time,
                          string.format("%02d:%02d:%02d", getDateTime().hour, getDateTime().min, getDateTime().sec),
                          rpm_buffer)
            rpm_buffer = {}  -- 清空缓冲区
        end
        ...
        if #current_buffer > 0 then
            write_current_data(cached_model_name, fly_number, current_start_time,
                              string.format("%02d:%02d:%02d", getDateTime().hour, getDateTime().min, getDateTime().sec),
                              current_buffer)
            current_buffer = {}  -- 清空缓冲区
        end

        -- 写入电压数据（在日志写入成功后）
        if #voltage_buffer > 0 then
            write_voltage_data(cached_model_name, fly_number, voltage_start_time,
                              string.format("%02d:%02d:%02d", getDateTime().hour, getDateTime().min, getDateTime().sec),
                              voltage_buffer)
            voltage_buffer = {}  -- 清空缓冲区
        end
    end
         ...
         if telemetry_initialized then
             ...
             if current_model_name and current_model_name ~= "" then
                 ...
                 local target_file_name = "[".. safe_model_name .."]" ..
                     ...
                     string.format("%02d", getDateTime().mon) ..
                     string.format("%02d", getDateTime().day) .. ".log"
                 local target_file_path = "/WIDGETS/DBK_T16SMK3Mini/logs/" .. target_file_name
